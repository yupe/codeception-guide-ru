...
* `steps` - все выполняемые действия будут выводится в консоль.
* `debug` - будет выводиться дполнительная отладочная информация.
* `config` - можно указать специальный конфигурационный файл для этого запуска.
* `colors` - включить цвета (если поддердивает терминал)
* `silent` - выодить только минимальную информацию.
